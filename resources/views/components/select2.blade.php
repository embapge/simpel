...
<div wire:ignore x-data="select2(@entangle($attributes->wire('model')).live)">
    <select style="width: 100%" x-ref="mySelect2" x-model='select2' {{ $attributes->get('inputAttributes') }}
        name="{{ $name }}" class="@error($name) is-invalid @enderror" id={{ $id }}>
            ...
            Alpine.data('select2', (model) => ({
                ...
                init() {
                    ...
                    $(ownSelect).select2({
                        ...
                        dropdownParent: $(ownSelect).parents(".modal").length ? $(ownSelect)
                            .parents(".modal") : false,
                        theme: 'bootstrap-5',
                        allowClear: true
